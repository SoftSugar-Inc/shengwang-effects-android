...
{
    const char *kStickerPath = "com/softsugar/stmobile/STMobileEffectNative";
    jclass cls = env->FindClass(kStickerPath);
    if(!cls) {
        LOGE("JNI OnLoad: failed to get %s class reference", kStickerPath);
        return NULL;
    }
    jfieldID fieldId = env->GetFieldID(cls, "mSoundPlay", "Lcom/softsugar/stmobile/STSoundPlay;");
...
{
    const char *kStickerPath = "com/softsugar/stmobile/STMobileStickerNative";
    jclass cls = env->FindClass(kStickerPath);
    if(!cls) {
        LOGE("JNI OnLoad: failed to get %s class reference", kStickerPath);
        return NULL;
    }
    jfieldID fieldId = env->GetFieldID(cls, "mStickerEvent", "Lcom/softsugar/stmobile/STStickerEvent;");
