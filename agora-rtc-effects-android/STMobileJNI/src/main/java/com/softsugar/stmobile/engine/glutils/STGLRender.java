...
public class STGLRender {
    ...
    private void initInner(int width, int height, int widthResize, int heightResize) {
        STLogUtils.i(TAG, "initInner() called with: width = [" + width + "], height = [" + height + "], widthResize = [" + widthResize + "], heightResize = [" + heightResize + "]");
    ...
    public void calculateVertexBuffer(int displayW, int displayH, int imageW, int imageH) {
        STLogUtils.i(TAG, "calculateVertexBuffer() called with: displayW = [" + displayW + "], displayH = [" + displayH + "], imageW = [" + imageW + "], imageH = [" + imageH + "]");
     ...
     */
    public int preProcess(int textureId, ByteBuffer buffer, boolean isFormatOes) {
        return preProcess(textureId, buffer, 0, isFormatOes);
    }
     ...
     * @return 转换后的GL_TEXTURE_2D的纹理id
     */
    public int preProcess(int textureId, ByteBuffer buffer, int bufIndex, boolean isFormatOes) {
        if (mFrameBuffers == null || !mIsInitialized)
            return OpenGLUtils.NO_TEXTURE;

        GLES20.glUseProgram(mArrayPrograms.get(isFormatOes ? 0 : 1).get(PROGRAM_ID));
        GlUtil.checkGlError("glUseProgram");

        mGLCubeBuffer.position(0);
        int glAttribPosition = mArrayPrograms.get(isFormatOes ? 0 : 1).get(POSITION_COORDINATE);
        GLES20.glVertexAttribPointer(glAttribPosition, 2, GLES20.GL_FLOAT, false, 0, mGLCubeBuffer);
        GLES20.glEnableVertexAttribArray(glAttribPosition);

        mTextureBuffer.position(0);
        int glAttribTextureCoordinate = mArrayPrograms.get(isFormatOes ? 0 : 1).get(TEXTURE_COORDINATE);
        GLES20.glVertexAttribPointer(glAttribTextureCoordinate, 2, GLES20.GL_FLOAT, false, 0, mTextureBuffer);
        GLES20.glEnableVertexAttribArray(glAttribTextureCoordinate);

        if (textureId != -1) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
            GLES20.glBindTexture(isFormatOes ? GLES11Ext.GL_TEXTURE_EXTERNAL_OES : GLES20.GL_TEXTURE_2D, textureId);
            GLES20.glUniform1i(mArrayPrograms.get(isFormatOes ? 0 : 1).get(TEXTURE_UNIFORM), 0);
        }
    ...
    public int preProcessAndResizeTexture(int textureId, boolean needResize, int[] resizedTextureId, ByteBuffer buffer, int bufferIndex, boolean isFormatOes) {
