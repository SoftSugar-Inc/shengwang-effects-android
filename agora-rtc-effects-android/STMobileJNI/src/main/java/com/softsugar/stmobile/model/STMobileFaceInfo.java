...
public class STMobileFaceInfo {
    public STMobile106 face106;           ///< \~Chinese 人脸信息，包含矩形框、106点、head pose信息等  \~English face q06 info

    public STPoint[] extraFacePoints;     ///< \~Chinese 眼睛、眉毛、嘴唇关键点. 没有检测到时为NULL  \~English extra face points
    public int extraFacePointsCount;      ///< \~Chinese 眼睛、眉毛、嘴唇关键点个数. 检测到时为ST_MOBILE_EXTRA_FACE_POINTS_COUNT, 没有检测到时为0 \~English extra face points count

    STPoint[] tonguePoints;    ///< \~Chinese 舌头关键点数组 \~English tongue points
    float[] tonguePointsScore; ///< \~Chinese 舌头关键点对应的置信度[0,1] 值越大，置信度越高 \~English tongue points score
    int tonguePointsCount;     ///< \~Chinese 舌头关键点的数目 \~English tongue points count

    STPoint[] eyeballCenter;       ///< \~Chinese 眼球中心关键点. 没有检测到时为NULL \~English eyeball center key points
    public int eyeballCenterPointsCount;  //< \~Chinese 眼球中心关键点个数. 检测到时为ST_MOBILE_EYEBALL_CENTER_POINTS_COUNT, 没有检测到时为0 \~English eyeball center key points count

    STPoint[] eyeballContour;      ///< \~Chinese 眼球轮廓关键点. 没有检测到时为NULL \~English eyeball contour key points
    public int eyeballContourPointsCount; ///< \~Chinese 眼球轮廓关键点个数. 检测到时为ST_MOBILE_EYEBALL_CONTOUR_POINTS_COUNT, 没有检测到时为0 \~English eyeball contour key points count

    float leftEyeballScore;        ///< \~Chinese 左眼球检测结果（中心点和轮廓点）置信度: [0, 1.0], 建议阈值0.95 \~English left eyeball score
    float rightEyeballScore;       ///< \~Chinese 右眼球检测结果（中心点和轮廓点）置信度: [0, 1.0], 建议阈值0.95 \~English right eyeball score

    long faceAction;                ///< \~Chinese 脸部动作 \~English face action
    private float[] faceBlendShapes;    // \~Chinese 面部表情动画置信度 \~English face blend shape info
    private int blendShapesCount;    // \~Chinese 面部表情动画的数目 \~English face blend shape count

    float[] faceActionScore;        ///< \~Chinese 各脸部动作置信度:eye, mouth, pitch, yaw, brow \~English face action score
    int faceActionScoreCount;       ///< \~Chinese 脸部动作个数 \~English face action score count

    byte[] avatarHelpInfo;          ///< \~Chinese avatar辅助信息,仅限内部使用，严禁修改 \~English avatar help info
    int avatarHelpInfoLength;       ///< \~Chinese avatar辅助信息字节长度 \~English avatar help info length

    public int skin_type;           ///< \~Chinese 肤色 \~English skin color
    public STColor hairColor;       ///< \~Chinese 发色 \~English hair color

    STFaceExtraInfo faceExtraInfo;  ///< \~Chinese 人脸extra \~English face extra info

    public STPoint3f gazeDirection[];     ///< \~Chinese 视线方向 \~English gaze direction
    public float gazeScore[];             ///< \~Chinese 视线置信度: [0.0, 1.0], 建议阈值为0.5 \~English gaze score
    public STFaceMesh faceMesh;        ///< \~Chinese 3d mesh信息，包括3d mesh关键点及个数 \~English face mesh info
    public STMobileEarInfo earInfo;       ///< \~Chinese 耳朵信息，包括耳朵关键点及个数，左右耳置信度 \~English ear info
    public STMobileForeheadInfo foreheadInfo; ///< \~Chinese 额头点信息，包括额头点坐标和个数 \~English forehead info
    private ST3DPose facePose;// \~Chinese 人脸新pose3d信息 \~English face pose 3d info
    ...
    public void setFaceBlendShapes(float[] faceBlendShapes) {
    ...
    public void setBlendShapesCount(int blendShapesCount) {
        this.blendShapesCount = blendShapesCount;
    }
    ...
    public void setFacePose(ST3DPose facePose) {
