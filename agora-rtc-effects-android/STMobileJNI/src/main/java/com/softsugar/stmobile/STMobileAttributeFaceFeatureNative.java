...
import com.softsugar.stmobile.model.STAttributeFaceFeature;
...
public class STMobileAttributeFaceFeatureNative {
     ...
     * \~Chinese 删除已添加的子模型
     *
     * @param config \~Chinese 要删除的子模型对应的config，config为创建人体行为检测句柄配置选项（例如：ST_MOBILE_ENABLE_BODY_KEYPOINTS）
     * @return \~Chinese 成功返回0，错误返回其他，参考STCommon.ResultCode
     */
    public native int removeSubModelByConfig(int config);

    /**
     * \~Chinese 检测人脸106点及人脸行为
     *
     * @param imgData       \~Chinese 用于检测的图像数据
     * @param imageFormat   \~Chinese 用于检测的图像数据的像素格式,比如STCommon.ST_PIX_FMT_NV12。能够独立提取灰度通道的图像格式处理速度较快，
     *                      \~Chinese 比如ST_PIX_FMT_GRAY8，ST_PIX_FMT_YUV420P，ST_PIX_FMT_NV12，ST_PIX_FMT_NV21
