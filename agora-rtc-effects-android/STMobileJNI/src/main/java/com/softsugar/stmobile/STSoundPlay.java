...
import com.softsugar.stmobile.model.STGanRequest;
...
public class STSoundPlay {
    private static final String TAG = "STSoundPlay";

    private final String CACHED_FOLDER = "Audio";
    private String mCachedPath;
    private final Context mContext;
    private MediaPlayer mediaPlayer;
    private final HashMap<String, SoundMetaData> mSoundMetaDataMap = new HashMap<>();
    ...
    /**
     * \~Chinese 音频播放监听器 \~English sound play control listener
     */
    public interface PlayControlListener {
        /**
         * \~Chinese 加载音频素材callback
         *
         * @param name    \~Chinese 音频名称
         * @param content \~Chinese 音频内容
         *
         * \~English sound load callback
         *
         * @param name    \~English name
         * @param content \~English sound data
         */
        void onSoundLoaded(String name, byte[] content);

        /**
         * \~Chinese 播放音频callback
         *
         * @param name \~Chinese 音频名称
         * @param loop \~Chinese 循环次数，0表示无限循环，直到onStopPlay回调，大于0表示循环次数
         *
         * \~English sound start play callback
         *
         * @param name \~English sound name
         * @param loop \~English loop count
         */
        void onStartPlay(String name, int loop);

        /**
         * \~Chinese 停止播放callback
         * @param name \~Chinese 音频名称
         *
         * \~English stop sound play
         * @param name \~English sound name
         */
        void onStopPlay(String name);

        /**
         * \~Chinese 暂停播放callback
         * @param name \~Chinese 音频名称
         *
         * \~English sound pause callback
         * @param name \~English sound name
         */
        void onSoundPause(String name);

        /**
         * \~Chinese 重新播放callback
         * @param name \~Chinese 音频名称
         *
         * \~English sound resume callback
         * @param name \~English sound name
         */
    ...
    /**
     * \~Chinese gan image监听器 \~English gan image listener
     */
    public interface GanControlListener {

        void onGanRequest(STGanRequest ganRequest, STEffectModuleInfo moduleInfo);
    }

    public void setGanControlListener(GanControlListener listener) {
        if (listener != null) {
            mGanControlDefaultListener = listener;
        }
    }

    /**
     * \~Chinese SenseArPlay初始化
     * @param appContext \~Chinese application context
     *
     * \~English sound Play init
     * @param appContext \~English application context
     */
    ...
    public void setEffectHandle(STMobileEffectNative effectHandle) {
        if (null == effectHandleRef) {
            effectHandleRef = new WeakReference<>(effectHandle);
        }
    }

    /**
     * \~Chinese 释放资源 \~English release sound resource
     */
    public void release() {
        if (mediaPlayer != null) {
            mediaPlayer.release();
     ...
     *
     * \~English custom PlayControlListener
     * @param listener \~English custom listener
     */
    ...
    //JNI调用，不做混淆
    private void onGanRequest(STGanRequest request, STEffectModuleInfo moduleInfo) {
        if (mGanControlDefaultListener != null) {
            mGanControlDefaultListener.onGanRequest(request, moduleInfo);
        }
    }
...
//                mAudioManager.abandonAudioFocus(afChangeListener);
    ...
    private PlayControlListener mPlayControlDefaultListener = new PlayControlListener() {
        ...
        public void onStartPlay(String name, int loop) {
            ...
            try {
                Log.d(TAG, "setDataSource onStartPlay Path: " + mCachedPath + File.separator + name);
...
//            }
    ...
    private GanControlListener mGanControlDefaultListener = new GanControlListener() {

        @Override
        public void onGanRequest(STGanRequest ganRequest, STEffectModuleInfo moduleInfo) {
            Log.e(TAG, "ganRequest " + ganRequest);
            if (ganRequest != null) {
                Log.e(TAG, "onGanRequest: "+ganRequest.getCategory() );
            }
        }
    };
}
